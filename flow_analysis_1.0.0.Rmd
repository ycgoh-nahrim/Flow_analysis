---
title: "Flow analysis for average daily flow"
...
params:
  ...
  SF_filename: 
    ...
    input: file
    value: "5606410_SgMuda_thian.csv"
  Water_year_month:
...
This Rmarkdown document was created to simplify processing and analysis of daily average streamflow data. Most of the methods used in this analysis is in accordance with [Manual on Low-flow Estimation and Prediction](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf) by World Meteorological Organization (WMO, 2008). 
...
Station number: **`r station_no`**

Station name: **`r station_name`**


Select **average daily streamflow** data in **csv** format. csv should have **2 columns**, with headers **Date** and **Flow** (case sensitive). Flow is in **m^3^/s**.
...
Remove null values (-999) and other signs (?). Reformat data for analysis.
...
> For the time series of daily mean flow data, it is not uncommon for an entire record to be rejected if, for example, more than 20 per cent of the values are missing. For a  single year’s daily data, thresholds of 5 per cent (more than 18 days of data missing) or 10 per cent are often used. 
>
> [Manual on Low-flow Estimation and Prediction - Section 3.3.1](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf)
...
day_w_data <- length(unique(flowdata_sel$Date))
...
gg_flowdata_cnt <- flowdata_cnt2 %>%
  ...
  labs(title = "Streamflow record count by year")
...
The records for station **`r paste0(station_no, " ", station_name)`** starts from year **`r min_year_all`** to **`r max_year_all`**. Of the **`r total_year`** years, a total of **`r year_w_data`** years (**`r day_w_data`** days) with complete* records (defined here as having more than **`r min_cnt`** records per year) from year **`r min_year`** to **`r max_year`** are used in the analysis. Only years with complete* records are selected for analysis to reduce bias due to seasonal effect.
...
Frequency distribution of streamflow data (complete* years).
...
Long-term daily streamflow hydrograph for all available data (including data from incomplete years). 
...
Flow_avg_mth <- Flow_avg_mth_all %>%
  group_by(month) %>%
  summarise(mth_Flow = mean(avg_Flow))
...
>
> [Manual on Low-flow Estimation and Prediction - Section 5.2](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf)


The BFI for station **`r paste0(station_no, " ", station_name)`** is **`r round(bfi_data_sel, 2)`** for selected data and **`r round(bfi_data, 2)`** for all available data. The BFI and baseflow separation is calculated using the package `lfstat` and is based on [WMO (2008)](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf).
...
flowdata_bf %>%
  ...
  scale_x_date(name= "Year", date_labels = "%Y",
               date_breaks = "year",
               minor_breaks = NULL) + #x axis format
  scale_y_continuous(name= expression(paste("Baseflow (", m^3, "/s )")),
                     minor_breaks = NULL) + #y axis format
  theme(text=element_text(family=font_family, color="grey20"),
        panel.grid.major.x = element_blank(),
        axis.text.x = element_text(angle = 90, hjust = 0.5)) +
...
Calculated from selected complete* years.
...
box_flow_chart <- Flow_avg_mth_all %>%
  ...
  scale_x_continuous(name = "Month",
                     ...
                     minor_breaks = NULL, 
                     labels=month.abb) + #x axis format
  scale_y_continuous(name= paste("Monthly Flow (m<sup>3</sup>/s )"),
                     #breaks = seq(0, 350, by = 50), 
                     minor_breaks = NULL) + #y axis format
...
Here the FDC is calculated from selected complete* years.
...
>
> [Manual on Low-flow Estimation and Prediction - Chapter 7](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf)

Three most commonly used durations are considered here, which are 1, 7 and 30 days duration. Moving average of all three durations are calculated and the annual minimum series are extracted.
...
>
> [Manual on Low-flow Estimation and Prediction - Section 7.3](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf)
...
>
> [Manual on Low-flow Estimation and Prediction - Section 7.5](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf)
...
>
> [Department of Irrigation and Drainage Malaysia (DID). (2015). *Hydrological Procedure No. 12: Magnitude and Frequency of Low Flows in Peninsular Malaysia*](http://h2o.water.gov.my/man_hp1/HP_12_Revised.pdf)
...
>
> [Manual on Low-flow Estimation and Prediction - Section 7.8](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf)
...
The results can be sorted by clicking on the header of interest.
...
Compare RMSE for all durations, all distributions.
...
Check work directory for output.
...
[Manual on Low-flow Estimation and Prediction](http://www.wmo.int/pages/prog/hwrp/publications/low-flow_estimation_prediction/WMO%201029%20en.pdf)
